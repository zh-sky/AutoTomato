...
def get_suite(i):
    ...
    with open(file_path, 'wb') as f:
        runner = HTMLTestRunner.HTMLTestRunner(stream=f, title='番茄来了测试报告', description='测试版的哦哦哦', verbosity=2)
        driver = get_driver(i)
        suite = unittest.TestSuite()
        # print(ParameTestCase.parametrize(LoginCase, driver=driver))
        # suite.addTest(ParameTestCase.parametrize(LoginCase, driver=driver))
        suite.addTest(ParameTestCase.parametrize(RoomStatusCase, driver=driver))
        runner.run(suite)
